...
export default function HomeNewsletterSection() {
  ...
  return (
    <section id="newsletter-preview" className="w-full border-b border-gray-100 bg-gray-50">
      <div className="container-fluid max-w-7xl px-4 py-12 md:py-16">
        <motion.div
          ...
          {posts.map((p, i) => (
            ...
            >
              <Link
                ...
                className="flex h-full flex-col rounded-2xl border border-gray-200 bg-white p-6 transition-all hover:border-primary/40 hover:shadow-sm"
              >
                ...
                <time dateTime={p.date} className="mt-5 block text-xs text-gray-400">
                  {p.date.replace(/-/g, ".")}
                </time>
              </Link>
            </motion.div>
