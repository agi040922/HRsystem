...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { post } = await getBoardPost(resolvedParams.id, 'newsletter')
  ...
  return {
    title: post.meta_title || `${post.title} | 뉴스레터 | FAIR인사노무컨설팅`,
...
export default async function NewsletterDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params
  const { post, error } = await getBoardPost(resolvedParams.id, 'newsletter')
